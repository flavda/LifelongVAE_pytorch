...
parser.add_argument('--normalization_classifier', type=str, default='batchnorm',
...
def execute_graph(epoch, model, fisher, data_loader, grapher, optimizer=None, prefix='test'):
    ...
    for data, labels in data_loader:
        ...
        with torch.no_grad() if 'train' not in prefix else dummy_context():
            ...
            output_map = model(data, labels)  # output of student_teacher_classifier forward
            loss_t = model.loss_function(output_map, fisher)
            loss_t['accuracy_mean'] = softmax_accuracy(output_map['student']['y_hat_logits'],
                                              output_map['augmented']['labels'], size_average=True)
    ...
    print('{}[Epoch {}][{} samples]: Average loss: {:.4f}\tELBO: {:.4f}\tKLD: {:.4f}\tNLL: {:.4f}\tMut: {:.4f} '
          '\tClassif: {:.4f}\tAccuracy: {:.4f}'.format(
    ...
    loss_val = {'loss_mean': loss_map['loss_mean'].detach().item(),
                ...
                'accuracy_mean': loss_map['accuracy_mean'].detach().item(),
